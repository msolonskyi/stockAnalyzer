...
as
    vv_module_name common.log.program%type := 'обработка цен';
    vn_rowcount    number(8) := 0;
begin
    common.pkg_log.sp_log_message('начало.', vv_module_name);
    -- 1. средневзвешенна€ цена акции
    ...
    common.pkg_log.sp_log_message('средневзвешенна€ цена акции', vv_module_name, vn_rowcount);
    -- 2. минимальна€ цена акции
    merge into indicator_values iv
    using (select trades.ticker, periods.id as period_id, periods.name,
                  min(price) value
           from trades, periods
    ...
    vn_rowcount := SQL%rowcount;
    common.pkg_log.sp_log_message('минимальна€ цена акции', vv_module_name, vn_rowcount);
    -- 3. максимальна€ цена акции
    merge into indicator_values iv
    using (select trades.ticker, periods.id as period_id, periods.name,
                  max(price) value
           from trades, periods
    ...
    vn_rowcount := SQL%rowcount;
    common.pkg_log.sp_log_message('максимальна€ цена акции', vv_module_name, vn_rowcount);
    -- 4. последн€€ цена акции
    merge into indicator_values iv
    using (select trades.ticker, periods.id as period_id, periods.name,
    ...
    vn_rowcount := SQL%rowcount;
    common.pkg_log.sp_log_message('последн€€ цена акции', vv_module_name, vn_rowcount);
    -- 5. commit
    commit;
