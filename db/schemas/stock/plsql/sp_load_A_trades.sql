...
as
    vv_module_name common.log.program%type := 'load trades from ux.ua (A)';
        ...
        if ((sysdate - nvl(vt_ent_table(indx).db, common.pkg_const.sf_get_const_first_price_date())) >= 1) then
            begin
                ...
                if ((sysdate - nvl(vt_ent_table(indx).db, common.pkg_const.sf_get_const_first_price_date())) >= 10) then
                    vv_end_dtm := to_char(nvl(vt_ent_table(indx).db, common.pkg_const.sf_get_const_first_price_date()) + vn_days, 'ddmmyyyy');
                else
                    vv_end_dtm := to_char(sysdate, 'ddmmyyyy');
                end if;
                vv_value := 'http://mdata.ux.ua/qdata.aspx?code='||vt_ent_table(indx).ticker||'&'||'pb='||to_char(nvl(vt_ent_table(indx).db, common.pkg_const.sf_get_const_first_price_date()) + 1, 'ddmmyyyy')||'&'||'pe='||vv_end_dtm||'&'||'p=0'||'&'||'mk=1'||'&'||'ext=0'||'&'||'sep=2'||'&'||'div=2'||'&'||'df=5'||'&'||'tf=2'||'&'||'ih=0';
